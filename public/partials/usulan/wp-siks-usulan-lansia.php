...
<div class="pb-4 mb-5">
    <h1 class="text-center my-4">Data Usulan Lansia</h1>
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="judulmodalTambahData">Tambah Data Lansia</h5>
                <h5 class="modal-title" id="judulModalEdit">Edit Data Lansia</h5>
...
<script>
        ...
        if (typeof tableWrse === 'undefined') {
            window.tableWrse = jQuery('#tableData').on('preXhr.dt', function(e, settings, data) {
                ...
                "ajax": {
                    ...
                    dataType: 'json',
                    data: {
                        'action': 'get_datatable_data_usulan_lansia',
                        'api_key': ajax.apikey,
                        'id_desa': id_desa
                    }
                },
    ...
    function hapus_data(id) {
        ...
        if (confirmDelete) {
            ...
            jQuery.ajax({
                ...
                type: 'post',
                data: {
                    'action': 'hapus_data_usulan_lansia_by_id',
                    'api_key': ajax.apikey,
                    'id': id
                },
                dataType: 'json',
    ...
    function edit_data(id) {
        ...
        jQuery.ajax({
            ...
            dataType: 'JSON',
            data: {
                'action': 'get_data_usulan_lansia_by_id',
                'api_key': ajax.apikey,
                'id': id,
            },
            success: function(res) {
                ...
                jQuery('#statusKehidupanRumahTangga').val(res.data.status_kehidupan_rumah_tangga);

                jQuery('#statusKepersertaanProgramBansos').val(res.data.status_kepersertaan_program_bansos);
                jQuery('#rekomendasiPendataLama').val(res.data.rekomendasi_pendata_lama);
                jQuery('#keteranganLainnyaLama').val(res.data.keterangan_lainnya_lama);
                jQuery('#rekomendasiPendata').val(res.data.rekomendasi_pendata);
                jQuery('#keteranganLainnya').text(res.data.keterangan_lainnya);

                jQuery('#statusDtks').val(res.data.status_dtks);
    ...
    function showModalTambahData() {
        ...
        jQuery('#statusKehidupanRumahTangga').val('');
        jQuery('#statusDtks').val('');
        jQuery('#lampiran').val('');
        jQuery('#tahunAnggaran').val('');
        jQuery('#keteranganLainnya').val('');

        jQuery('#statusKepersertaanProgramBansos').val('');
        jQuery('#rekomendasiPendataLama').val('');
        jQuery('#keteranganLainnyaLama').val('');
        jQuery('#rekomendasiPendata').val('');
        jQuery('#keteranganLainnya').text('');
    ...
    function submitVerifikasi() {
        ...
        tempData.append('jenis_data', 'lansia');
    ...
    function submitData() {
        ...
        const tempData = new FormData();
        tempData.append('action', 'tambah_data_usulan_lansia');
        tempData.append('api_key', ajax.apikey);
