function relayAjax(options, retries = 20, delay = 5000, timeout = 9000000) {
	...
	options.cache = false;
	jQuery.ajax(options).fail(function (jqXHR, exception) {
		// console.log('jqXHR, exception', jqXHR, exception);
		if (jqXHR.status != "0" && jqXHR.status != "503" && jqXHR.status != "500") {
...
function import_excel_disabilitas(action = "", message = "") {
	var jenis_import = jQuery(
		'input[name="carbon_fields_compact_input[_crb_jenis_disabilitas]"]:checked'
	).val();
	if (jenis_import == "") {
...
function import_excel_bunda_kasih(action = "", message = "") {
	import_excel(
		"import_excel_bunda_kasih",
		"Success import data Bunda Kasih dari excel!"
	);
}

function import_excel_anak_terlantar(action = "", message = "") {
	var jenis_import = jQuery(
		'input[name="carbon_fields_compact_input[_crb_jenis_anak_terlantar]"]:checked'
	).val();
	if (jenis_import == "") {
		return alert("Jenis data belum dipilih!");
	}
...
function import_excel_p3ke_siks() {
	import_excel(
		"import_excel_p3ke_siks",
		"Success import data P3KE dari excel!"
	);
}

function import_excel_calon_penerima_p3ke_siks() {
	import_excel(
		"import_excel_calon_p3ke_siks",
...
function import_excel(action = "", message = "") {
	if (action == "") {
		...
		if (!data) {
			...
			var update_active = prompt(
				"Apakah anda mau menonaktifkan data sebelumnya? ketik 1 jika iya dan kosongkan saja jika tidak."
			);
